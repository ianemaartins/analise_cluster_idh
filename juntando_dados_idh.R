...
#CALCULAR TAXA DE MORTALIDADE A CADA 100.000 HABITANTES
  ...
  #Agrupar dados do IDH de cada estado
  dados_idh_estados <- dados_idh_estados %>%
    left_join(tabela_idh_est, by = "Sigla")
...
#CALCULAR TAXA DE MORTALIDADE A CADA 10.000 HABITANTES
  ...
  # Converter CODMUNRES para character em tabela_idh_mun
  tabela_idh_mun <- tabela_idh_mun %>%
    mutate(CODMUNRES = as.character(CODMUNRES))
  
  #Agrupar dados do IDH de cada municipio
  dados_idh_municipios <- dados_idh_municipios %>%
    left_join(tabela_idh_mun, by = "CODMUNRES")
  
  #Remover NA
  dados_idh_municipios <- na.omit(dados_idh_municipios)
